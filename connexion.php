...
//$reponse = $bdd->query('SELECT id FROM utilisateurs');
    ...
    var_dump($d);
    if(isset($d['pseudo'])) {
      $_SESSION['user-connecté'] = $d['pseudo'];
      echo "connexion réussie";
      if($_SESSION['user-connecté'] == "admin"){
        header('Location: confirmuser.php?pseudo='.$_SESSION['user-connecté']);
      }else{
        //$_SESSION['user-connecté'] = $d['id_user'];
        header('Location: user.php?pseudo='.$_SESSION['user-connecté']);
      }
    }
    else{
